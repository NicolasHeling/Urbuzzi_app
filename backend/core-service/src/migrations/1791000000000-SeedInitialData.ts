...
/**
 * Migration de Seed: Insere dados iniciais de lotes e propostas
 * apenas se as tabelas estiverem vazias.
 *
 * Extraído do main.ts para seguir boas práticas de Migrations.
 */
export class SeedInitialData1791000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Só insere se a tabela lots estiver vazia
    const [{ count }] = await queryRunner.query(`SELECT COUNT(*) AS count FROM lots`);
    if (parseInt(count, 10) > 0) {
      console.log('[Seed] Tabela lots já possui dados — seed ignorado.');
      return;
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DELETE FROM proposals WHERE id IN (
    ...
    await queryRunner.query(`DELETE FROM lots WHERE id IN (
